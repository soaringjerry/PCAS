...
CGO_ENABLED=0 go test -v ./internal/storage/sqlite/... || exit 1
...
CGO_ENABLED=0 go build -mod=vendor -tags 'netgo no_chroma' -o /tmp/pcas-pure ./cmd/pcas || exit 1
echo "✅ Pure Go build successful"
echo

echo "4. Building with CGO (if needed for comparison)..."
CGO_ENABLED=1 go build -mod=vendor -tags 'cgo no_chroma' -o /tmp/pcas-cgo ./cmd/pcas || exit 1
